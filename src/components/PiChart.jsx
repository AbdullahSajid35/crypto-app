...
const renderCustomizedLabel = ({
    ...
    return (
      ...
      >
        ...
        <tspan className='font-bold text-[20px]' x={x} dy="1.2em">%{(percent * 100).toFixed(1)}</tspan> {/* Adjusted dy */}
      </text>
    );
  };
    ...
    const getData=async()=>{
        ...
        if (res?.data?.status==='success'){
            const new_data=[
                ...
                { name: "DİĞER", value: 100-res?.data?.data?.btc_dominance-res?.data?.data?.eth_dominance }
            ]
