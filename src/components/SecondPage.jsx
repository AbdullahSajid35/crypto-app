...
function SecondPage() {
  ...
  const getData = async () => {
    try {
      const [res, res_gainers, res_loosers, res_month_gainers, res_month_loosers] = await Promise.all([
      ...
      if (res?.data?.status === 'success') {
        ...
        if (bitcoinData) {
          const { percent_change_7d: percentage, percent_change_30d: monthPercent } = bitcoinData?.quote?.USD ?? {};
        ...
        if (ethereumData) {
          const { percent_change_7d: percentage, percent_change_30d: monthPercent } = ethereumData?.quote?.USD ?? {};
  ...
  return (
    <div className='py-2 text-center px-8 sm:px-16 flex flex-col gap-3 '>
      <h1 className='text-[20px] sm:text-[30px] md:text-[44px] text-center font-bold text-white' ><span className='text-orange'>HAFTALIK</span> VE <span className='text-orange'>AYLIK</span> YÜKSELENLER <span className='text-orange'>/</span> DÜŞENLER</h1>
